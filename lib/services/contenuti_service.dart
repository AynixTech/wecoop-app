...
/// Client per contenuti pubblici del backend Node (posts, partners, leads…).
///
/// Ex-`WordpressService`: i dati non passano più da WordPress REST.
class ContenutiService {
  ...
  Future<List<Post>> getPosts({int perPage = 5}) async {
    try {
      ...
      AppLogger.d('getPosts: status ${response.statusCode}');
      return [];
    } catch (e) {
      AppLogger.e('getPosts errore', e);
      return [];
    }
  }
  ...
  // Recupera le offerte formative attive
  ...
  // Invia richiesta "Studiare in Italia"
  ...
  // Invia richiesta generica (es. "Vivere in Italia")
...
/// Alias legacy — preferire [ContenutiService].
@Deprecated('Usa ContenutiService')
typedef WordpressService = ContenutiService;
